...
*/ 
PetscErrorCode PetscTanHWeightingSeparated(PetscReal *Result, PetscReal phi, PetscReal Amplitude, PetscReal Offset)
{
  PetscReal weight;
  weight = 0.5 - 0.5 * PetscTanhReal((PetscAbsReal(phi)-Offset) * Amplitude);
  Result[0] = weight;
  Result[1] = 1.0 - weight;
  PetscFunctionReturn(0);
}
...
PetscErrorCode PetscGaussianNucleationTime(PetscReal *Result, PetscReal time, PetscReal T)
{
  PetscReal weight;
  if (time<T)
  {
    weight = PetscExpReal((time-T)*(time-T)/(time*(time-2.0*T)));
  } else {
    weight = 1.0;
  }
  Result[0] = weight;
...
PetscErrorCode FaultSDFInit_v3(SpecFECtx c, PetscInt geometry_selection)
{
  PetscErrorCode  ierr;
  PetscInt        e,i,q,nbasis;
  Vec             coor;
  const PetscReal *LA_coor;
  PetscInt        *element,*elnidx,*eldofs;
  PetscReal       *elcoords;
  ...
  /** SDF Initialization
   * Creation,
   * Possible fill in approach of acquiring the SDF at every quadrature point per element in the grid.
  */ 
  PetscPrintf(PETSC_COMM_WORLD,"Start SDF Init (v3.1): ");
...
{
  PetscReal x0 = fabs(DistOnFault)-(NucleationHalfLength-5.0*delta);
  PetscReal y0 = Phi;

  *OutCorner = PETSC_FALSE;
  if (fabs(DistOnFault) < (NucleationHalfLength-5.0*delta)){
    *OutCorner = PETSC_TRUE;
    PetscFunctionReturn(0);
  }

  if ((x0*x0/25.0 + y0*y0) < (delta*delta)){
    *OutCorner = PETSC_TRUE;
  }
  ...
  for (e=0; e<c->ne; e++) {
    ...
    for (q=0; q<c->nqp; q++) {
      ...
      */
      ...
      if (fabs(DistOnFault) < 1.5*1.0e3 && inside_fault_region) {
      ...
      if (inside_fault_region) { 
        ...
        //(sigma_n < 0) 
        { 
          PetscReal WeightedValues[] = {0,0};
          PetscReal BlendParamA = blend_a_factor/c->delta;
                                                                  ...
                                                                  PetscReal blend_a_factor, PetscReal blend_phio_factor)
{
  ...
  /* compute and stress stress */
  for (e=0; e<c->ne; e++) {   
    PetscBool Cell_inside_fault_region = PETSC_FALSE; 
    /* get element -> node map */
    elnidx = &element[nbasis*e];
    ...
    for (i=0; i<nbasis; i++) {
      ierr = FaultSDFQuery(&elcoords[2*i], c->delta, the_sdf, e*nbasis + i, &Cell_inside_fault_region);CHKERRQ(ierr);

      if (Cell_inside_fault_region) {
        PetscReal DistOnFault;
        DistOnFault = 0.0;

        ierr = evaluate_DistOnFault_sdf(the_sdf, &elcoords[2*i], e*nbasis + i, &DistOnFault);CHKERRQ(ierr);

        if (fabs(DistOnFault) < Half_L_nuc){
  ...
  for (e=0; e<c->ne; e++) {
    ...
    for (q=0; q<c->nqp; q++) {
      ...
      PetscBool inside_fault_region;
      PetscReal DistOnFault; /**Project coords onto fault to get friction in the case of tilting */
      PetscReal sigma_trial[3],gradu[4];
      PetscReal e_vec[]={0,0,0},sigma_vec[]={0,0,0};
...
{
  PetscErrorCode ierr;
  PetscInt  e,nqp,q,i,nbasis;
  PetscReal e_vec[3],gradv[4];
  PetscInt  *element,*elnidx,*eldofs;
...
PetscErrorCode se2dr_demo(PetscInt mx,PetscInt my)
{
  ...
  */
 {
    ...
    blend_a_factor = 12.0;
    blend_phio_factor = 0.65;
    ierr = PetscOptionsGetReal(NULL,NULL,"-blend_a_factor",&blend_a_factor,NULL);CHKERRQ(ierr);
    ierr = PetscOptionsGetReal(NULL,NULL,"-blend_phio_factor",&blend_phio_factor,NULL);CHKERRQ(ierr);

    switch (ModType){
      case KOSTROV:
        PetscPrintf(PETSC_COMM_WORLD,"   [Model Params] || Kostrov Model\n");
        break;
      case TPV3:
        PetscPrintf(PETSC_COMM_WORLD,"   [Model Params] || TPV3 Model\n");
        break;
    }

    PetscPrintf(PETSC_COMM_WORLD,"[Blending factor] ||   A   = %1.4e\n", blend_a_factor);
    PetscPrintf(PETSC_COMM_WORLD,"[Blending factor] || Phi_o = %1.4e\n", blend_phio_factor);
      
    geometry_type = 1;// Set the type of SDF used (third argument of the function): 0-> Horizontal, 1-> Tilted, 2->Sigmoid
    // If tilted choose the angle in rupture.h
    ierr = PetscOptionsGetInt(NULL,NULL,"-geometry_type",&geometry_type,NULL);CHKERRQ(ierr);
 }
  
  ierr = FaultSDFInit_v3(ctx, geometry_type);CHKERRQ(ierr);
  ...
  */
  switch (ModType){
    case KOSTROV:
      ierr = SpecFECtxSetConstantMaterialProperties_Velocity(ctx,4000.0 ,4000.0/sqrt(3.0), 2500.0);CHKERRQ(ierr); // vp,vs,rho (Kostrov)
      break;
    case TPV3:
      ierr = SpecFECtxSetConstantMaterialProperties_Velocity(ctx,6000.0 ,3464.0, 2670.0);CHKERRQ(ierr); // vp,vs,rho (TPV3)
      break;
  }
  ...
  */
  ...
  if (ctx->sdf->type == 2)
  {
    ierr = PetscFree(ctx->sdf->idxArray_ClosestFaultNode);CHKERRQ(ierr);
    ierr = PetscFree(ctx->sdf->idxArray_SDFphi);CHKERRQ(ierr);
    ierr = PetscFree(ctx->sdf->idxArray_DistOnFault);CHKERRQ(ierr);
  }
  ierr = PetscFree(ctx->sdf->xList);CHKERRQ(ierr);
  ierr = PetscFree(ctx->sdf->fxList);CHKERRQ(ierr);


  ierr = PetscFree(ctx->elbuf_field);CHKERRQ(ierr);
  ierr = PetscFree(ctx->elbuf_field2);CHKERRQ(ierr);
  ierr = PetscFree(ctx->elbuf_field3);CHKERRQ(ierr);
  ierr = PetscFree(ctx->elbuf_coor);CHKERRQ(ierr);
  ierr = PetscFree(ctx->dr_qp_data);CHKERRQ(ierr);
  ierr = PetscFree(ctx->elbuf_dofs);CHKERRQ(ierr);
  ierr = PetscFree(ctx->cell_data);CHKERRQ(ierr);


  ierr = PetscFree(ctx->w);CHKERRQ(ierr);
  ierr = PetscFree(ctx->w1d);CHKERRQ(ierr);
  ierr = PetscFree(ctx->xi1d);CHKERRQ(ierr);
  ierr = PetscFree(ctx->element);CHKERRQ(ierr);
  ierr = PetscFree(ctx->sigma);CHKERRQ(ierr);


  //ierr = GeoParamsDestroy((GeometryParams*)ctx->sdf->data);CHKERRQ(ierr);
  //ierr = SDFDestroy(&ctx->sdf);CHKERRQ(ierr);
