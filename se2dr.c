...
#if 0
PetscErrorCode PointLocation_v1(SpecFECtx c,const PetscReal xr[],PetscInt *_eid,PetscReal N[])
{
  static PetscBool beenhere = PETSC_FALSE;
  static PetscReal      gmin[3],gmax[3];
  PetscReal      dx,dy;
  PetscInt       k,ei,ej,nid,eid;
  PetscInt       nbasis,i,j;
...
PetscErrorCode FaultSDFInit_v1(SpecFECtx c)
{
  PetscErrorCode  ierr;
  ...
  for (e=0; e<c->ne; e++) {
    ...
    for (q=0; q<c->nqp; q++) {
      ...
      if (modify_stress_state) {
        ...
        PetscReal normal[2],tangent[2],Vplus,Vminus,slip_rate;
  ...
  printf("[Fault point location-v1] Finished\n");
...
PetscErrorCode FaultSDFTabulateInterpolation_v1(SpecFECtx c,const PetscReal LA_v[],DRVar *dr_celldata_q,PetscReal v_plus[],PetscReal v_minus[])
{
  PetscInt  eid_delta,*element_delta;
  PetscReal vx_d_e,vy_d_e;
  PetscInt i,nbasis,*element;
  ...
  for (i=0; i<nbasis; i++) {
    PetscInt nidx = element_delta[i];
    
    vx_d_e = LA_v[2*nidx  ];
    vy_d_e = LA_v[2*nidx+1];
    v_plus[0] += dr_celldata_q->Ni_0[i] * vx_d_e;
    v_plus[1] += dr_celldata_q->Ni_0[i] * vy_d_e;
  }
  ...
  for (i=0; i<nbasis; i++) {
    ...
    vx_d_e = LA_v[2*nidx  ];
    vy_d_e = LA_v[2*nidx+1];
    v_minus[0] += dr_celldata_q->Ni_1[i] * vx_d_e;
    v_minus[1] += dr_celldata_q->Ni_1[i] * vy_d_e;
  }
  PetscFunctionReturn(0);
}
#endif
...
PetscErrorCode AssembleLinearForm_ElastoDynamics_StressGlut2d(SpecFECtx c,Vec u,Vec v,PetscReal dt,Vec F)
{
  PetscErrorCode ierr;
  PetscInt  e,nqp,q,i,nbasis,ndof;
  PetscReal e_vec[3],sigma_vec[3],sigma_trial[3],gradu[4],gradv[4],gradv_q[5*5][4];
  PetscInt  *element,*elnidx,*eldofs;
      ...
      if (inside_fault_region) {
        ...
        if (sigma_n < 0) { /* only consider inelastic corrections if in compression */
          ...
          if (T > tau) {
            ...
            //if (e==20100 && q==2)printf("  [e %d,q %d] x_qp -> slip_rate %+1.4e\n",e,q,slip_rate);
...
PetscErrorCode RecordDRVar_MultipleStations_NearestGLL_SEQ(SpecFECtx c,PetscReal time,PetscInt nr,PetscReal xr[])
{
  ...
  if (!beenhere) {
    ...
    fprintf(fp,"#   time(1)\n");
    for (r=0; r<nr; r++) {
      PetscInt offset = 1 + r*3; /* 1 is for time */
      
      fprintf(fp,"#     slip(%d) sliprate(%d) mu(%d) -> station [%d]\n",offset+1,offset+2,offset+3,r);
    }
    ierr = VecRestoreArrayRead(coor,&LA_c);CHKERRQ(ierr);
    beenhere = PETSC_TRUE;
  } else {
  ...
  for (r=0; r<nr; r++) {
    ...
    fprintf(fp," %+1.8e %+1.8e %+1.8e",cell_data[q_index].slip,cell_data[q_index].slip_rate,cell_data[q_index].mu);
  }
  fprintf(fp,"\n");
...
PetscErrorCode se2dr_demo(PetscInt mx,PetscInt my)
{
  ...
  */
  {
    ...
    PetscReal scale[] = {  4.0*alpha, 4.0*alpha };
    PetscReal shift[] = { -2.0*alpha,-2.0*alpha };
  ...
  */
  ierr = SpecFECtxSetConstantMaterialProperties_Velocity(ctx,6000.0 ,3434.0, 2670.0);CHKERRQ(ierr); // vp,vs,rho
  ...
  */
  ...
  for (k=1; k<=nt; k++) {
    ...
    ierr = AssembleLinearForm_ElastoDynamics_StressGlut2d(ctx,u,v,dt,f);CHKERRQ(ierr);
